...
function App() {
  ...
  const [center, setCenter] = useState<Point>({} as Point);

  useEffect(() => {
    setCenter(pointsFixed[0]);
    if (enable) {
      const interval = setInterval(() => {
        setPoints((prev) => {
          return [...prev, pointsFixed[prev.length]];
        });
      }, 500);
      return () => clearInterval(interval);
      // setPoints(pointsFixed);
    }
  }, [enable]);
    ...
    <>
      ...
      >
        {center.lat && center.lng && (
          <HereMap apikey={apikey} points={points} center={center} />
        )}

        <button onClick={() => setEnable((prev) => !prev)}>Enable</button>
      </div>
    </>
  );
}
